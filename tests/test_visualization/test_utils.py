...
def test_create_pairs():
    ...
    pairs2 = create_pairs(df=df, numeric_cols=['A', 'B'])
    assert len(pairs) != 0
    assert len(pairs1) == 0,"only columns A and B in df are numeric"
    assert len(pairs2) != 0, "only numeric cols are supplied"
